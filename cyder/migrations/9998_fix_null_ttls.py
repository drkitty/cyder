...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # The DNS builder now uses a model's own TTL instead of its SOA's TTL.

        simple_models = (
            orm.AddressRecord, orm.CNAME, orm.MX, orm.Nameserver, orm.SRV,
            orm.SSHFP, orm.StaticInterface, orm.TXT)

        for model in simple_models:
            for pk, new_ttl in model.objects.filter(ttl=0).values_list(
                    'pk', 'domain__soa__ttl'):
                model.objects.filter(pk=pk).update(ttl=new_ttl)
        for pk, new_ttl in orm.PTR.objects.filter(ttl=0).values_list(
                'pk', 'reverse_domain__soa__ttl'):
            orm.PTR.objects.filter(pk=pk).update(ttl=new_ttl)

        # MySQL complains if we try to make a column NOT NULL when it
        # contains NULL values.

        for model in simple_models + (orm.PTR,):
            model.objects.filter(ttl__isnull=True).update(ttl=3600)
